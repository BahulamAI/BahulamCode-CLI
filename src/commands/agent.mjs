...
/**
 * Main entry point for all `b0 agent` subcommands.
...
function printAgentUsage() {
    ...
    process.stderr.write(`  ${CYAN}b0 agent list${RESET}                           List user-defined agents\n`);
    process.stderr.write(`  ${CYAN}b0 agent get <slug>${RESET}                     Show agent details\n`);
    process.stderr.write(`  ${CYAN}b0 agent sync [--dir <path>]${RESET}            Sync agent YAML files\n`);
...
async function handleGet(args) {
    ...
    if (!slug) {
        process.stderr.write(`${RED}✗ Usage: b0 agent get <slug>${RESET}\n`);
        process.exit(1);
    }
